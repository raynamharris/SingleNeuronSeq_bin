...
sample_id <- dir(file.path(base_dir,"05_kallistoquant_MusUCSD_2016-03-25"))
sample_id
kal_dirs <- sapply(sample_id, function(id) file.path(base_dir, "05_kallistoquant_MusUCSD_2016-03-25", id))
kal_dirs

s2c <- read.csv("2016-03-26-sample_annotation.csv", sep=",", header = TRUE, stringsAsFactors=FALSE)
...
s2c <- filter(s2c, tissue %in% c("tissue"))
s2c$condition <- NULL
s2c$condition <-(c('1_ng','100_pg','1_ng','100_pg','10_pg','100_pg','10_pg','10_pg'))
s2c <- subset(s2c, select=c(1,7,2,3,4,5,6))
...
so <- sleuth_fit(so)
so <- sleuth_wt(so, 'condition10_pg') 
models(so)
sleuth_live(so)
