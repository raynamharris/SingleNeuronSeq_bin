...
### Read data for samples and counts
load("/Users/raynamharris/Github/SingleNeuronSeq/results/2016-02-25_results/2016-02-25_02-dplyrmutate.Rdata")


samples <- read.csv("/Users/raynamharris/Github/SingleNeuronSeq/data/2016-02-25-sample_annotation.csv", header=TRUE, sep=",")

## Deseq take "colData" and "countData" as inputs. 
## Let's start with an analysis of the full data set
countData <- counts_rename_mutate
str(countData)
head(countData)
colData <- samples
str(colData)
...
## This particular dataset needs some datawrangling. 
## This block of code makes all the numbers integers


countData <- sapply(countData, as.integer)
...
################## DESeq2 ################## 
## or just load previously run data 
# load("~/Github/SingleNeuronSeq/results/2016-02-24-results/2016-02-18_04-DESeq.Rdata")


## all results will go here 
setwd("~/Github/SingleNeuronSeq/results/2016-02-25_results")
...
dds <- DESeqDataSetFromMatrix(countData, colData, design= ~ RNAseqBatch + condition) 
...
dds <- DESeq(dds)
save.image("/Users/raynamharris/Github/SingleNeuronSeq/results/2016-02-25_results/2016-02-25_04-DESeq.Rdata")
...
res <- results(dds, contrast = c("condition", "neuron.bio", "tissue.dillution"))
...
## comparing results for two diff single neuron comparison
res2 <- results(dds, contrast = c("condition", "neuron.bio", "neuron.tech"))
...
res3 <- results(dds, contrast = c("condition", "tissue.dillution", "neuron.tech"))
res3 <- res2[order(res3$padj),]
head(res3)
mcols(res3,use.names=TRUE)
summary(res3)

write.csv(as.data.frame(res2),file="res3.csv")
